 ...
 * real on this machine as the current user, rooted at the project directory.
 *
 * Ported from kobe `packages/kobe/src/engine/ai-sdk/local-sandbox.ts`
 * (same author's sibling project). `workRoot` is required — codefox always
 * roots a session at a project directory, never a tmpdir.
 *
 * Running as the user is the whole point: the harness bridge's runtime picks
 * up the locally-installed CLI's login state (Claude/Codex subscription), so
 * no API key enters the picture. There is NO isolation by design — the
 * "sandbox" IS the host.
 */
...
export function createLocalSandbox({ workRoot }: { workRoot: string }): HarnessV1SandboxProvider {
  return {
    ...
    providerId: "codefox-local-host-sandbox",
